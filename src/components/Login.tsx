...
export default function Login({ onLoginSuccess }: LoginProps) {
  ...
  const [step, setStep] = useState<'mobile' | 'passkey' | 'set-passkey'>('mobile');
  ...
  const handleMobileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingUser && existingUser.passkey) {
        setStep('passkey');
      } else {
        setStep('set-passkey');
      }
  ...
  const handlePasskeySubmit = async (e: React.FormEvent) => {
    ...
    if (step === 'passkey') {
      if (passkey === currentUser.passkey) {
        onLoginSuccess(currentUser);
      } else {
        setError('Incorrect passkey.');
      }
    } else if (step === 'set-passkey') {
      if (passkey.length !== 4) {
      ...
      const updatedUser = { ...currentUser, passkey: passkey };
      try {
      ...
      <div className="mt-5 sm:mx-auto sm:w-full sm:max-w-xs">
        <div className="bg-white dark:bg-slate-900 py-5 px-4 shadow-sm border border-slate-200/80 dark:border-slate-800 rounded-xl">
          ...
          >
            ...
            <form onSubmit={step === 'mobile' ? handleMobileSubmit : handlePasskeySubmit} className="space-y-5" id="form-login-credentials">
              ...
              {(step === 'passkey' || step === 'set-passkey') && (
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 tracking-wide uppercase" htmlFor="login-passkey">
                    {step === 'passkey' ? 'Enter Passkey' : 'Set 4-Digit Passkey'}
                  </label>
                  <div className="relative rounded-xl shadow-xs">
                    <input
                      id="login-passkey"
                      ...
                      className="block w-full px-4 py-2.5 text-xs border border-slate-300 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-950/40 text-slate-850 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-600 dark:focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 transition disabled:opacity-60"
                      placeholder="XXXX"
                      maxLength={4}
                    />
                  </div>
              ...
              {step === 'set-passkey' && (
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 tracking-wide uppercase" htmlFor="login-confirm-passkey">
                    Confirm Passkey
                  </label>
                  <div className="relative rounded-xl shadow-xs">
                    <input
                      id="login-confirm-passkey"
                      type="password"
                      required
